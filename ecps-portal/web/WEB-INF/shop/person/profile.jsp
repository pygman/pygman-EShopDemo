...
<title>个人资料_用户中心_移动商城_中国移动通信</title>
...
$(function(){
	...
	/**
		ajaxSubmit提交表单必须要放在submit(function(){
			...
			return false
		})
		阻止表单提交
		...
		$(this).ajaxSubmit({
			...
			success:function(responseText){
				if(responseText == "success"){
					...
					window.location.href = "${path}/user/toLogin.form";
				}
			},
			error:function(){
				alert("系统错误");
...
<div class="w loc">
	...
	<p class="r"><a href="index.jsp" title="首页">首页</a><samp>&gt;</samp><a href="../person/index.jsp" title="用户中心">用户中心</a><samp>&gt;</samp><span class="gray">个人资料</span></p>
...
<div class="w mt ofc">
	<div class="l wl">
		...
		<div class="box bg_gray">
			<ul class="ul left_nav">
			<li><a href="../person/profile.jsp" title="个人资料" class="here">个人资料</a></li>
	...
	<div class="r wr profile">
		...
		<div class="confirm">
			...
			<div class="ofc">
				...
				<form id="jvForm" action="${path }/user/updateUser.form" method="post">
					<input type="hidden" name="ptlUserId" value="${user.ptlUserId }">
					<ul class="uls form">
					<li id="errorName" class="errorTip" style="display:none">${error}</li>
					
					<li>
						<label for="username">用 户 名：</label>
						<span class="word">${user.username }</span>
					</li>
					<li>
					...
					<li>
						...
						<span class="bg_text"><input type="text" value="${user.realName }" id="realName" name="realName" maxLength="32" vld="{required:true}" /></span>
						<span class="pos"><span class="tip okTip">&nbsp;</span></span>
					</li>
					<li>
						<label for="gender">性　　别：</label>
						<span class="word">
							<input type="radio" name="gender" value="0" <c:if test="${user.gender == 0 }">checked</c:if>  />&nbsp;男
							<input type="radio" name="gender" value="1" <c:if test="${user.gender == 1 }">checked</c:if>/>&nbsp;女
						</span>
					</li>
					<li>
						<label for="telphone">联系电话：</label>
						<span class="bg_text"><input type="text" id="phone" value="${user.phone }" name="phone" maxLength="32" vld="{required:true}" /></span>
						<span class="pos"><span class="tip warningTip">用户名为4-20位字母、数字或中文组成，字母区分大小写。</span></span>
					</li>
					<li>
						<label for="residence">居 住 地：</label>
						<span class="word">
						<select name="resiProv">
							<option value="" selected>省/直辖市</option>
							<option value="广西" <c:if test="${user.resiProv=='广西' }">selected</c:if> >广西</option>
							<option value="吉林" <c:if test="${user.resiProv=='吉林' }">selected</c:if>>吉林</option>
						</select>
						<select name="resiCity">
							<option value="" selected>城市</option>
							<option value="南宁" <c:if test="${user.resiCity=='南宁' }">selected</c:if>>南宁</option>
							<option value="长春" <c:if test="${user.resiCity=='长春' }">selected</c:if>>长春</option>
						</select>
						<select name="resiDist">
							<option value="" selected>县/区</option>
							<option value="清秀区" <c:if test="${user.resiDist=='清秀区' }">selected</c:if>>清秀区</option>
							<option value="二道区" <c:if test="${user.resiDist=='二道区' }">selected</c:if>>二道区</option>
						</select>
						</span>
					</li>
					<li><label for="address">详细地址：</label>
						<span class="bg_text"><input type="text" id="addr" value="${user.addr }" name="addr" maxLength="32" vld="{required:true}" /></span>
						<span class="pos"><span class="tip errorTip">用户名为4-20位字母、数字或中文组成，字母区分大小写。</span></span>
					</li>
					<li>
						<label for="zipCode">邮政编码：</label>
						<span class="bg_text"><input type="text" id="zipCode" value="${user.zipCode }" name="zipCode" maxLength="32" vld="{required:true}" /></span>
					</li>
