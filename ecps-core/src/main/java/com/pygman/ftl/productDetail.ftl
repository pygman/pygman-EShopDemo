...
<#assign image_path="http://localhost:8088/ecps-image"/>
...
$(function(){
	...
	$(".spec a").click(function(){
		...
		$("#skuPriceB").html("￥"+skuPrice);
		$("#marketPrice").html("￥"+marketPrice);
		$.ajax({
			url:"${path}/item/validStock.form",
			type:"post",
			dataType:"text",
			data:{
				skuId:skuId
	...
	//使用程序触发指定的事件，事件一定是在触发之前定义好的
	$(".spec a:first").trigger("click");
...
<div class="w loc">
	...
	<dl id="cart" class="cart l">
		<dt><a href="${path}/cart/listCart.form" title="结算">结算</a>购物车:<b id="">123</b>件</dt>
		<dd class="hidden">
...
<div class="w ofc mt"> 
	...
	<div class="r wr">
		...
		<div class="product">
			...
			<div class="showPro">
				<div class="big"><a id="showImg" class="cloud-zoom" href="${image_path }${item.imgSize1}" rel="adjustX:10,adjustY:-1"><img title="optional title display" alt="" src="${image_path }${item.imgSize1}"></a></div>
				<div class="small">
					<span class="smallL" title="向左">&nbsp</span>
					<div class="smallBox">
						<div class="smallList">
							<a class="cloud-zoom-gallery here" title="red" href="${image_path }${item.imgSize1}" rel="useZoom: 'showImg', smallImage: '${image_path }${item.imgSize1}'"><img alt="thumbnail 1" src="${image_path }${item.imgSize1}"></a>
							<a class="cloud-zoom-gallery" title="blue" href="${image_path }${item.imgSize1}" rel="useZoom: 'showImg', smallImage: '${image_path }${item.imgSize1}'"><img alt="thumbnail 2" src="${image_path }${item.imgSize1}"></a>
							<a class="cloud-zoom-gallery" title="blue" href="${image_path }${item.imgSize1}" rel="useZoom: 'showImg', smallImage: '${image_path }${item.imgSize1}'"><img alt="thumbnail 3" src="${image_path }${item.imgSize1}"></a>
							<a class="cloud-zoom-gallery" title="blue" href="${image_path }${item.imgSize1}" rel="useZoom: 'showImg', smallImage: '${image_path }${item.imgSize1}'"><img alt="thumbnail 4" src="${image_path }${item.imgSize1}"></a>
							<a class="cloud-zoom-gallery" title="blue" href="${image_path }${item.imgSize1}" rel="useZoom: 'showImg', smallImage: '${image_path }${item.imgSize1}'"><img alt="thumbnail 5" src="${image_path }${item.imgSize1}"></a>
						</div>
					</div>
			...
			<form method="post" action="" name="" class="infor">
				...
				<li><label>移 动 价：</label><span class="word"><b id="skuPriceB" class="f14 red mr">￥3999</b>(市场价:<del id="marketPrice">￥5789.00</del>)</span></li>
				...
				<div class="box_orange">
					...
					<div class="pre spec">
						<#list item.skuList as sku>
								<#if sku_index == 0>
									<a href="javascript:void(0);"  class="here" skuId="${(sku.skuId)?c }" skuPrice="${sku.skuPrice }" marketPrice="${sku.marketPrice }">
										<#list sku.specList as spec>
											${spec.specValue }
										</#list>
									</a>
								<#else>
									<a href="javascript:void(0);"  skuId="${(sku.skuId)?c }" skuPrice="${sku.skuPrice }" marketPrice="${sku.marketPrice }">
										<#list sku.specList as spec>
											${spec.specValue }
										</#list>
									</a>
					...
					<li><label>我 要 买：</label><a href="javascript:void(0);" class="inb sub"></a><input readonly type="text" name="" id="quantity" value="1" class="num" size="3" /><a href="javascript:void(0);" class="inb add"></a><em id="sub_add_msg" class="red"></em></li>
					<li class="submit">
