...
<head>
<title>订单详情_已付款单_<fmt:message key="OrderMgmtMenu.title" /></title>
...
<body id="main">
	...
	<form id="completeForm" action="${path }/order/completeTask.do" method="post">
		<input type="hidden" name="outcome" id="buttonName">
		<input type="hidden" name="taskId" value="${tb.taskId }">
		<input type="hidden" name="orderId" value="${tb.order.orderId }">
	</form>
				...
				<form id="myForm" action="${path}/order/orderPay/oper.do"
					...
					<div class="sch page_c">
						<span class="l">订单号：<b class="red"><var>${tb.order.orderNum}</var></b>&nbsp;&nbsp;&nbsp;下单时间：<var><fmt:formatDate value="${tb.order.orderTime}"pattern="yyyy-MM-dd HH:mm:ss" /></var>&nbsp;&nbsp;&nbsp;<b class="f14 blue"></b>&nbsp;&nbsp;&nbsp; 
								...
								<c:forEach items="${tb.comeouts }" var="outcome">
									<input name="r" type="button" id="completeCall" class="pointer sub1 red" value="${outcome }" />
								</c:forEach>							 
