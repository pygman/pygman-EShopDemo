...
$(function(){
	...
	$("#confirmDivOk").bind("click",function(){
		$.ajax({
			...
			url : '${path}/order/valetOrderCancel.form?orderNum='+orderNum+'&taskId='+taskId ,
			dataType : 'json',
...
<div class="loc icon"><samp class="t12"></samp><fmt:message key='menu.current.loc'/>：代客下单管理&nbsp;&raquo;&nbsp;<span class="gray" title="">查询修改订单</span></div>
    ...
    <form action="${path}/order/listGuestOrder.form" id="form1" name="form1" method="post">
   ...
   <table cellspacing="0" summary="" class="tab" id="myTable">
    ...
    <tr>
    <td>${p.areaName}</td>
    <td><a href="${path}/order/orderSummary/detail.form?orderId=${p.orderId}&taskId=${p.taskId}">${p.orderNum}</a></td>
   ...
   <td class="nwp"><ui:orderState var="${p.orderState}" type="1"/></td>
    <td><a href="${path}/order/orderSummary/detail.form?orderId=${p.orderId}&taskId=${p.taskId}&flag=1">查看</a>
    <c:choose>
    	<c:when test="${p.orderState<6}">
    		<a href="${path}/order/orderSummary/editOrder.form?orderId=${p.orderId}&taskId=${p.taskId}">修改</a>
    	</c:when>
    	<c:otherwise>
