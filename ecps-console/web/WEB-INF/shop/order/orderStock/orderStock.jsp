...
function exportOrder(s){
    ...
    form.action="${path}/order/orderExport.form?s="+s+"&type=2&type2=12";
    subVerify4Title($("#orderNo"));
    subVerify4Title($("#phone"));
    subVerify4Title($("#shipName"));
    subVerify4Title($("#userName"));
    form.submit();
    form.action="${path}/order/orderStock.form";
}
// $(function(){
...
<div class="frameR"><div class="content">
    <div class="loc icon"><samp class="t12"></samp><fmt:message key='menu.current.loc'/>：<fmt:message key="OrderMgmtMenu.title"/>&nbsp;&raquo;&nbsp;<span class="gray" title="备货中单">备货单</span></div>
     <h2 class="h2_ch">
      <span id="tabs" class="l">
        <a id="label1" href="${path}/order/orderStock.form?orderState=2,4,5,6,7,8,21,18,10,11,12,13,19,20,14,22,23,24,25"   title="全部" class="nor">全部</a>
        <%--
        <ui:permTag src="/ecps/console/order/orderStock.form?orderState=2">
         --%>
        <a id="label3" href="${path}/order/orderStock.form?orderState=2"   title="备货中" class="nor">待备货</a>
        <%--
        </ui:permTag>
         --%>
       
       <%-- <ui:permTag src="/ecps/console/order/orderStock.form?orderState=4">
       --%>
        <a id="label2" href="${path}/order/orderStock.form?orderState=4"   title="待打包" class="nor">待打包</a>  
        <%-- </ui:permTag>--%>
        <%-- <ui:permTag src="/ecps/console/order/orderStock.form?orderState=5">--%>
        <a id="label4" href="${path}/order/orderStock.form?orderState=5"   title="待物流取货" class="nor">待物流取货</a>  
		 <%--</ui:permTag>--%>
		  <a id="label5" href="${path}/order/orderStock.form?orderState=6,7,8,21,18,10,11,12,13,19,20,14,22,23,24,25"   title="取货完成" class="nor">取货完成</a>
      </span>
      <span class="r inb_a">
    ...
    <form action="${path}/order/orderStock.form" id="form1" name="form1" method="post">
...
<table cellspacing="0" summary="" class="tab" id="myTable">
    ...
    <tr>
    <td>${p.areaName}</td>
    <td><a href="${path}/order/orderStock/detail.form?orderId=${p.orderId}&taskId=${p.taskId}">${p.orderNum}</a></td>
    <td>${p.shipName}</td>
    <td>
    	<c:choose>
    ...
    <td><a href="${path}/order/orderStock/detail.form?orderId=${p.orderId}&taskId=${p.taskId}">查看</a></td>
    </tr>
    </c:forEach>
    </table>
<div class="page_c">
