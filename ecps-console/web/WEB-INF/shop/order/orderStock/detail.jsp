...
function test(){
	$.ajax({
		type : "POST",
		async : false,
		url : "${path}/order/queryAjaxSCode.form?seriescode="+$("#seriesCode").val(),
		success : function(responseText) {
			var dataObj = eval("(" + responseText + ")");
			var txt = dataObj[0].txt;
			if(txt==true){
...
function getIccid(serviceNum, areaCode) {
	$.ajax({
		type : "POST",
		async : false,
		url : "${path}/order/preemptSimCardRes.form",
...
<form id="myForm" action="${path}/order/orderStock/oper.form" method="post">
 ...
 物流编号:<c:out value="${ebco.ebOrder.deliveryNo}"/>
    ...
    <ui:permTag src="/ecps/console/order/orderStock/oper.form">
   <c:if test="${ebco.ebOrder.orderState == 4 && oprType !=1}">     
	...
	<span class="r"><input type="button" class="pointer" onclick="window.open('${base}/order/preview.form?orderId=${ebco.orderId}&type=12','_blank','height=300,width=500,scrollbars=yes,location=no,resizable=yes')" value="打印订单" /><input type="button" class="pointer" onclick="window.history.back(-1);" value="返回列表"/></span>
	</h2>
