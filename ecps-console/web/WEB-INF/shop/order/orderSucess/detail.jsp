...
<head>
<title>订单详情_已收货可退款_<fmt:message key="OrderMgmtMenu.title"/></title>
...
$(function(){
	...
	$("#but1").click(function(){
		...
		if(isSuccess == 'false'){
		    ...
		    if(oprType=="2"){
		    	window.location.href="${path}/order/orderValid.form?orderState=7,42"
		    	
		    } else {
		    	window.location.href="${path}/order/orderSucess.form?orderState=7,42"
		    }
		}
	});
...
function getBzqd(spid) {
	$.ajax({
		type : "POST",
		async : false,
		url : "${path}/order/packetAjaxList.form?itemNo="+spid,
...
function crm() {
	...
	$.ajax({
		type : "POST",
		async : false,
		url : "${path}/order/cancelAjaxCrm1.form",
		data:{
			orderNum:orderNum,
			oprType:oprType,
			note:note,
...
<div class="loc icon">
    ...
    <samp class="t12"></samp><fmt:message key='menu.current.loc'/>：<fmt:message key='OrderMgmtMenu.title'/>&nbsp;&raquo;&nbsp;已收货单&nbsp;&nbsp;<span class="gray" title="订单详情"></span>
    </c:if>
</div>
<form id="myForm" action="${path}/order/orderSucess/oper.form" method="post">
   <input type="hidden" name="taskId" id="taskId" value="${taskId}"/>
    ...
    <input type="hidden" name="isSuccess" id="isSuccess" value="true"/>

	<div class="sch page_c">
	   	<span class="l">订单号：<b class="red"><var>${ebco.ebOrder.orderNum}</var></b>&nbsp;&nbsp;&nbsp;下单时间：<var><fmt:formatDate value="${ebco.ebOrder.orderTime}" pattern="yyyy-MM-dd HH:mm:ss" /></var>&nbsp;&nbsp;&nbsp;<b class="f14 blue"><ui:orderModule var="${ebco.ebOrder.orderState}" type="18" /></b>&nbsp;&nbsp;&nbsp;
		 ...
		 <ui:permTag src="/ecps/console/order/orderSucess/oper.form">
		 <c:if test="${oprType != '1'&&ebco.ebOrder.orderType!=2}">
		<input type="button" name="r" class="pointer" id="r" value="退货退库"/></c:if>
		</ui:permTag>
		</span>
	</div>
	...
	<a href="javascript:void(0);" ref="#tab_3" title="操作记录" class="nor">操作记录</a>
	 ...
	 <span class="r"><input type="button" class="pointer" onclick="window.open('${base}/order/preview.form?orderId=${ebco.orderId}&type=7','_blank','height=300,width=500,scrollbars=yes,location=no,resizable=yes')" value="打印订单" /><input type="button" class="pointer" onclick="window.history.back(-1);" value="返回列表"/></span>
	</h2>
	...
	<!-- 金额合计 -->
		<div class="page_c">
			<span class="r">商品金额合计：<fmt:formatNumber value="${ebco.ebOrder.orderSum/100}" pattern="#0.00"/>元&nbsp;&nbsp;&nbsp;
					 运费：0.00元 &nbsp;&nbsp;&nbsp; 应退金额： <var class="f16 red"><fmt:formatNumber value="${ebco.ebOrder.orderSum/100}" pattern="#0.00"/>元</var>
			</span>
		</div>
