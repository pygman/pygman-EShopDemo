...
<head>
<title>已收货_<fmt:message key="OrderMgmtMenu.title"/></title>
...
// 	 case "":$("#label1").attr("class","here");break;
	 case "7,42":$("#label2").attr("class","here");break;
// 	 case "24,25,26":$("#label3").attr("class","here");break;
// 	 case "10,20,22":$("#label5").attr("class","here");break;
// 	 case "11,14,23":$("#label4").attr("class","here");break;
	}
});
function exportOrder(s){
    var form = document.getElementById("form1");
    form.action="${path}/order/orderExport.form?s="+s+"&type=2&type2=7";
    subVerify4Title($("#orderNo"));
    subVerify4Title($("#phone"));
    subVerify4Title($("#shipName"));
    subVerify4Title($("#userName"));
    form.submit();
    form.action="${path}/order/orderSucess.form";
}
</script>
...
<div class="frameR"><div class="content">
    <div class="loc icon"><samp class="t12"></samp><fmt:message key='menu.current.loc'/>：<fmt:message key="OrderMgmtMenu.title"/>&nbsp;&raquo;&nbsp;<span class="gray" title="已收货单">已收货单</span></div>
    <h2 class="h2_ch">
      <span id="tabs" class="l">
<%--         <a id="label1" href="${path}/order/orderSucess.form"   title="全部" class="nor">全部</a> --%>
        <a id="label2" href="${path}/order/orderSucess.form?orderState=7,42"   title="已收货" class="nor">已收货</a>  
<%--         <a id="label3" href="${path}/order/orderReturn.form?orderState=24,25,26"   title="待审核退款" class="nor">待审核退款</a> --%>
<%--         <a id="label5" href="${path}/order/orderReturn.form?orderState=10,20,22"   title="待退款" class="nor">待退款</a> --%>
<%--         <a id="label4" href="${path}/order/orderReturn.form?orderState=11,14,23" style="color:red;" title="已退款" class="nor">已退款</a> --%>
      </span>
       <span class="r inb_a">
    ...
    <form action="${path}/order/orderSucess.form" id="form1" name="form1" method="post">
    ...
    <tr>
    <td>${p.areaName}</td>
    <td><a href="${path}/order/orderSucess/detail.form?orderId=${p.orderId}&taskId=${p.taskId}">${p.orderNum}</a></td>
    <td>${p.shipName}</td>
    <td>
    	<c:choose>
    ...
    <td class="nwp"><ui:orderModule var="${p.orderState}" type="18"/></td>
    <td><a href="${path}/order/orderSucess/detail.form?orderId=${p.orderId}&taskId=${p.taskId}">查看</a></td>
    </tr>
    </c:forEach>
    </table>
    <div class="page_c">
