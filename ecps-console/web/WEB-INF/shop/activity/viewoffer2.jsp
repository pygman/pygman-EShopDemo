...
<div class="frameR"><div class="content">
    ...
    <div class="loc icon"><samp class="t12"></samp>当前位置：商品管理&nbsp;&raquo;&nbsp;<a href="<c:url value="/activity/listoffer.form"/>" >活动档次管理</a>&nbsp;&raquo;&nbsp;<span class="gray">档次审核</span><a href="<c:url value="/activity/listoffer.form"/>" class="inb btn120x20">返回活动档次</a></div>
           					...
           					<td>
           						<table>
           							...
           							<tr>
										...
										<td>
											<c:forEach items="${offer.listskuOffer }" var="skuOffer">
												<c:if test="${skuOffer.skuId==sku.skuId }">
													<c:set value="${skuOffer.discount }" var="discountPrice"/>
												</c:if>
											</c:forEach>
											<c:choose>
												<c:when test="${discountPrice==null || discountPrice==0 }">
													<input type="text" name="discoutPrice" value="" size="5" skuId="${sku.skuId }" disabled/>
												</c:when>
												<c:otherwise>
													<input type="text" name="discoutPrice" value="${discountPrice }" size="5" skuId="${sku.skuId }" disabled/>
												</c:otherwise>
											</c:choose>
										</td>
