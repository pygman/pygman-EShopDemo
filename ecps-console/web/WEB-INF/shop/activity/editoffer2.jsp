...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		function showResponse(responseText, statusText){ 
			...
			if(obj.result=="success"){
				if($('#opt').val()=='offer'){
					document.location.href="<c:url value='/ecps/console/activity/listoffer.form'/>";
				}else
					document.location.href="<c:url value='/ecps/console/activity/listoffer.form'/>?offerGroupId=${offer.offerGroupId }&offerType=2";
			}
		}
	});
	...
	function getData(){
		...
		$.ajax({
			type:"post",
			url:"${path}/activity/findItemForActivity.form",
...
/* 		var array    =tempValue.split(",");
	...
	function isHasOtherOffer(itemId){
		...
		$.ajax({
            type:"POST",
            async: false,
            url:"${path }/activity/isHasOtherOfferAjax.form",
            data:"itemId="+itemId,
	...
	function queryOfferProduct(skuId) {
		...
		$("#queryOfferProductQuery").click(function() {
			...
			$.ajax({
				type:"POST",
                async: false,
                url:"${path }/activity/queryOfferProduct.form",
                dataType:'json',
                data:{
               		offerId:offerId,
...
<div class="frameR"><div class="content">
    ...
    <div class="loc icon"><samp class="t12"></samp>当前位置:商品管理&nbsp;&raquo;&nbsp;<a href="${path}/activity/listoffer.form?offerGroupId=${offer.offerGroupId }&offerType=2" title="${offer.offerGroup.offerGroupName }的营销案档次">营销案档次</a>&nbsp;&raquo;&nbsp;<span class="gray">编辑档次</span><a href="${path}/activity/listoffer.form?offerGroupId=${offer.offerGroupId }&offerType=2" class="inb btn120x20" title="返回${offer.offerGroup.offerGroupName }的营销案档次">返回营销案档次</a></div>

    <form id="form111" name="form111" action="${path}/activity/updateOffer.form" method="post">
        <div class="edit set">
            ...
            <p><label>编号:</label>&nbsp;&nbsp;&nbsp;
             ...
             <p><label>档次编号(crm):</label>&nbsp;&nbsp;&nbsp;
            	...
            	<input type="text" name="condId" value="${offer.condId }" reg="^[0-9a-zA-Z]+$" tip="档次编号只能输入字母和数字" /><span></span>
            </p>
        ...
        <div class="edit set">
            ...
            <table cellspacing="0" summary="" class="tab">
                ...
                <tbody>
                    <tr>
                        <td>
                        	<samp>*</samp><input type="text" name="offerTerm" id="offerTerm" class="text20" maxlength="2" value="${offer.offerTerm}" size="5" info="业务协议期格式不正确" emptyTip="业务协议期不能为空"/>
                        </td>
                        ...
                        <td><input type="text" name="prepaid" value="<fmt:formatNumber value='${offer.prepaid }' pattern='#' maxFractionDigits='0'/>" class="text20" size="5" group="number" info="预存/赠送话费格式不正确" emptyTip="预存/赠送话费不能为空"/></td>
                        <td>
                ...
                <c:forEach items="${offer.listItem }" var="item">
                    <c:forEach items="${item.listsku }" var="sku" varStatus="skuIdx">
                        ...
                        <td>
                           ...
                           <%-- <input type='text' id='discoutPrice${sku.skuId }' name='discoutPrice' value="${sku.skuDiscount/100}" class='text20' size='5' /> --%> 
                        </td>
                        <td>
    ...
    <iframe src="${base}/consolelog/top10.form?entityId=${offer.offerId}&tableName=EB_OFFER" width="100%" height="400" marginwidth="0" marginheight="0" frameBorder="no" framespacing="0" allowtransparency="true" scrolling="auto"></iframe>
