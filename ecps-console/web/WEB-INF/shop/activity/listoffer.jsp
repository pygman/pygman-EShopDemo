...
/* 	    if(confirm("确定删除记录")){
	...
	function batchDel(){
	    ...
	    tipShow('#confirmDiv');
        $("input[id='confirmDivOk']").click(function(){
			$("#form1").attr("action","${base}/activity/delOffers.form");
	       	$("#form1").submit();
		})
/* 	    if(confirm("确定删除这些记录")){
	    	$("#form1").attr("action","${base}/activity/delOffers.form");
...
<div class="frameR"><div class="content">
    ...
    <div class="loc icon"><samp class="t12"></samp><fmt:message key='menu.current.loc'/>：<fmt:message key='ItemMgmtMenu.title'/>&nbsp;&raquo;&nbsp;<c:forEach items="${listgroup }" var="group"><c:if test='${group.offerGroupId==offer.offerGroupId}'><a href="<c:url value="/${system}/activity/listOfferGroup.form"/>">营销案管理</a>&nbsp;&raquo;&nbsp;</c:if></c:forEach><span class="gray"><c:forEach items="${listgroup }" var="group"><c:if test='${group.offerGroupId==offer.offerGroupId}'><c:out value="${group.offerGroupName }"/>的</c:if></c:forEach>营销案档次</span>
    </div>

    <form id="form1" name="form1" action="${base}/activity/listoffer.form" method="post">
	<div class="sch">
        ...
        <span class="r inb_a">
            <c:forEach items="${listgroup }" var="group">
                 <c:if test='${group.offerGroupId==offer.offerGroupId}'>
                        <c:url value="/ecps/console/activity/forwardAddactivity.form" var="addDC">
	...
	<table cellspacing="0" summary="" class="tab">
		...
		<tbody>
			...
			<tr>
				...
				<td>
					<c:url value="/ecps/console/activity/delOffer.form" var="delOffer">
						<c:param name="offerId" value="${offer.offerId}"/>	
					</c:url>
					<c:url value="/ecps/console/activity/getOffer.form?opt=offer" var="getOffer">
						<c:param name="offerId" value="${offer.offerId}"/>
						<c:param name="offerType" value="${offer.offerGroup.offerType}"/>
					</c:url>
