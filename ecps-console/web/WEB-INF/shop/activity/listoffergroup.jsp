...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$("input[id='addItemNoteConfirm']").click(function(){
	        ...
	        $.ajax({
	        	type:"post",
	         	url:"${base}/activity/updateOfferGroupStatus.form",
	         	data:(ajaxData),
	         	success:function(responseText){
	            	var result=eval("("+responseText+")");
	             	if(result._status=="true"){
	             		...
	             		alert("操作成功");
	             		window.location.href ="${base}/activity/listOfferGroup.form?showStatus=0&labelStatus=5";
	             	}else{
	             		alert(result._mes);
	             	}
	             	 tipHide('#addItemNote');
...
/* 	    if(confirm("确定删除记录")){
    ...
    function doIt(ajaxData,type){
        ...
        $.ajax({
             type:"POST",
             async: false,
             url:"${base}/activity/changeSatus.form",
             data:(ajaxData),
        ...
        if(result){
         ...
         if(!result){
             ...
             if(${labelStatus==0}) {
                   window.location.href ="${base}/activity/listOfferGroup.form?labelStatus=0";
            }
          if(${labelStatus==1}) {
                   window.location.href ="${base}/activity/listOfferGroup.form?auditStatus=0&labelStatus=1";
            }
          if(${labelStatus==2}) {
                   window.location.href ="${base}/activity/listOfferGroup.form?auditStatus=2&labelStatus=2";
            }
            if(${labelStatus==3}) {
                   window.location.href ="${base}/activity/listOfferGroup.form?auditStatus=1&labelStatus=3";

            }
            else if(${labelStatus==4})  {
           window.location.href ="${base}/activity/listOfferGroup.form?showStatus=1&labelStatus=4";
        }
    ...
    function publishOfferGroup(offerGroupId){
        ...
        $.ajax({
        	type:"post",
         	url:"${staticPublishUrlPre}/static4ebiz/static/publishSingleTopic.form",
         	data:(ajaxData),
         	success:function(responseText){
            	var result=eval("("+responseText+")");
             	if(result._status=="true"){
    ...
    function importOfferGroup() {
    	...
    	$.ajax({
    		type : "POST",
    		url : "${path}/activity/importOfferGroup.form",
    		success : function(responseText) {
    			...
    			$("#importResultOk").click(function(){
    				location.href = "<c:url value='/ecps/console/activity/listOfferGroup.form'/>?showStatus=1&labelStatus=4";
    			});
...
<form id="form1" name="form1" action="${base}/activity/listOfferGroup.form" method="post">

    <h2 class="h2_ch">
    <span id="tabs" class="l">
        <a href="${base}/activity/listOfferGroup.form?labelStatus=0" title="全部营销案" class="nor" id="label0" >全部</a>
        <a href="${base}/activity/listOfferGroup.form?showStatus=1&labelStatus=4"title="未上架营销案" class="nor" id="label4">未上架</a>
        <a href="${base}/activity/listOfferGroup.form?showStatus=0&labelStatus=5"title="已上架营销案" class="nor" id="label5">已上架</a>
    </span>
    </h2>
    ...
    <div class="page_c">
        ...
        <span class="r inb_a">
            ...
            <a href="${base}/activity/forwardAddOfferGroup.form"  class="btn80x20">添加营销案</a>
        </span>
    </div>
			...
			<tr>
				...
				<td>
					<c:choose>
						<c:when test="${offerSizeMap[offerGroup.offerGroupId].offerSize!=null}">
							<a href="${base}/activity/listoffer.form?offerGroupId=${offerGroup.offerGroupId}&offerType=${offerGroup.offerType}"><c:out value="${offerSizeMap[offerGroup.offerGroupId].offerSize}"/></a>
						</c:when>
				...
				<td>					
					<c:if test="${labelStatus==0||labelStatus==4||labelStatus==5}">
							<c:if test='${offerGroup.showStatus==0}'>
								<ui:permTag src="/${system}/activity/updateOfferGroupStatus.form">
	           ...
	           <td>
                    ...
                    <c:if test="${labelStatus==1 || labelStatus==2}">
                        <a href="${base}/activity/checkOfferGroup.form?offerGroupId=${offerGroup.offerGroupId}">审核</a>
                    </c:if>
                    
                    <a href="${base}/activity/viewOfferGroup.form?offerGroupId=${offerGroup.offerGroupId}" title="查看">查看</a>	
                    ...
                    <c:if test="${labelStatus==0}">
                    	...
                    	<c:if test='${offerGroup.showStatus==1}'>
                    		<a href="${base}/activity/getOfferGroup.form?offerGroupId=${offerGroup.offerGroupId}&labelStatus=0"><fmt:message key="tag.update"/></a>
                    	</c:if> 
                    </c:if>
                    <c:if test="${labelStatus==4}">
                    	...
                    	<c:if test='${offerGroup.showStatus==1}'>
                    		<a href="${base}/activity/getOfferGroup.form?offerGroupId=${offerGroup.offerGroupId}&showStatus=1&labelStatus=4"><fmt:message key="tag.update"/></a>
                    	</c:if> 
                    </c:if>
                   
                    <c:if test="${labelStatus==0||labelStatus==1||labelStatus==2||labelStatus==4 ||labelStatus==5}">
                    	<a href="${base}/activity/listoffer.form?offerGroupId=${offerGroup.offerGroupId}&offerType=${offerGroup.offerType}">查看档次</a>
                    </c:if>
                    <c:if test="${labelStatus==0||labelStatus==1||labelStatus==2||labelStatus==4}">
                        <a href="${base}/activity/forwardAddactivity.form?offerGroupId=${offerGroup.offerGroupId}&offerType=${offerGroup.offerType}">添加档次</a>
                    </c:if>
                    <c:if test="${labelStatus==0||labelStatus==1||labelStatus==2||labelStatus==3||labelStatus==4}">
                        <a href="javascript:void(0);" onclick="singleDel('${base}/activity/deleteOfferGroup.form?offerGroupId=${offerGroup.offerGroupId}')"><fmt:message key="tag.delete"/></a>
                    </c:if>
                    
                    <c:if test="${labelStatus==0}">
                    	<c:if test='${offerGroup.showStatus==0}'>
                    		<ui:permTag src="/${system}/static4ebiz/static/toPublishJsp.form">
