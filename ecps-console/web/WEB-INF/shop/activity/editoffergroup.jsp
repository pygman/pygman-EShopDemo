	...
	$(document).ready(function(){ 
	    ...
	    function validateData(formData, jqForm, options){
			...
			*/
			...
			$.ajax({
	        	type:"post",
	        	async: false,
	            url:"${base}/item/validActiveSen.form",
		...
		function showResponse(responseText, statusText){ 
			...
			if(obj.result=="success"){
				document.location.href="${base}/activity/listOfferGroup.form?labelStatus=${labelStatus}&showStatus=${offerGroup.showStatus}";	
			}
		}
...
<div class="frameR"><div class="content">
    ...
    <div class="loc icon"><samp class="t12"></samp><fmt:message key='menu.current.loc'/>：商品管理&nbsp;&raquo;&nbsp;<a href="${base}/activity/listOfferGroup.form" title="营销案管理">营销案管理</a>&nbsp;&raquo;&nbsp;<span class="gray">编辑营销案</span>
        <a href="${base}/activity/listOfferGroup.form" class="inb btn120x20">返回营销案管理</a></div>
    <form id="form1" name="form1" action="${base}/activity/updateOfferGroup.form" method="post">
         <div class="edit set">
         	<p><label>营销类型：</label>
         		<c:choose>
            ...
            <p><label><samp>*</samp>营销案名称：</label><input type="text" id="offerGroupName" name="offerGroupName" value="${offerGroup.offerGroupName }" reg="^[a-zA-Z0-9\u4e00-\u9fa5]{1,50}$" tip="必须是中英文或数字字符，长度1-50" class="text state" />
            	<span class="pos"><c:out value="${offerGroupName }"/></span>
            </p>
            <p><label>营销案编号：</label><input type="hidden" id="offerGroupNo" name="offerGroupNo" value="${offerGroup.offerGroupNo }" maxlength="8" class="text state"/>
            	<c:out value="${offerGroup.offerGroupNo}"/>
            </p>
            ...
            <p><label><samp>*</samp>营销案简称：</label><input type="text" name="shortName" value="${offerGroup.shortName }" reg="^[A-Z]{1,3}$" tip="只能是A-Z的一个至三个字母" class="text state" /> 
           		<span class="pos"></span>
            </p>
            <p><label>排序：</label><input type="text" name="offerGroupSort" value="${offerGroup.offerGroupSort}" reg="^[0-9]{0,3}$" tip="必须是1-3位数字" class="text small" />
            	<span class="pos"></span>
            </p>
            <p><label>页面关键词：</label><input type="text" id="keywords" name="keywords" value="${offerGroup.keywords }" reg="^.{0,50}$" tip="长度不能大于50字符" class="text state" />
            	<span class="pos"></span>
            </p>
            ...
            <input type='hidden' id='defaultImgAction' name='defaultImgAction' value='${base}/uploads/upload_pic.form' />
            ...
            <p><label><samp>*</samp>购买条件：</label>
            	...
            	<input type="checkbox" name="constrId" value="1"<c:if test='${doing1==1 }'> checked</c:if>/>神州行
            	<input type="checkbox" name="constrId" value="2"<c:if test='${doing2==2 }'> checked</c:if>/>全球通
            	<input type="checkbox" name="constrId" value="3"<c:if test='${doing3==3 }'> checked</c:if>/>动感地带
            </p>
            ...
            <p><label class="alg_t">页面描述：</label><textarea id="pageDesc" name="pageDesc" rows="4" cols="45" class="are" reg="^.{0,100}$" tip="长度不能大于100"><c:out value="${offerGroup.pageDesc }"/></textarea>
            	<span class="pos"></span>
            </p>
            <p><label>营销案详情地址：</label><input type="text" id="website" name="website" size="50" value="${offerGroup.website }" maxlength="100" class="text state" />
           		<span class="pos">请以http://开头</span>
            </p>
            <p><label class="alg_t">营销案简介：</label><textarea id="offerGroupIntro" name="offerGroupIntro" rows="4" cols="45" class="are" reg="^.{0,500}$" tip="长度不能大于500" ><c:out value="${offerGroup.offerGroupIntro}"/></textarea>
            	<span class="pos"></span>
            </p>
             <ui:permTag src="/${system}/offerGroup/review.form">
             <!-- 
            <p><label>审核状态：</label><input type="radio" id="auditStatus1" name="auditStatus" value="0"<c:if test='${offerGroup.auditStatus==0 }'> checked</c:if> />待审核&nbsp;&nbsp;<input type="radio" id="auditStatus2" name="auditStatus" value="1"<c:if test='${offerGroup.auditStatus==1 }'> checked</c:if> />审核通过&nbsp;&nbsp;<input type="radio" id="auditStatus3" name="auditStatus" value="2"<c:if test='${offerGroup.auditStatus==2 }'> checked</c:if> />审核不通过
             ...
             <ui:permTag src="/${system}/offerGroup/changeStatus.form">
            ...
            <input type="hidden" id="checkShowStatus" name="checkShowStatus" value="1"/>
             ...
             <p><label class="alg_t">操作备注：</label><textarea id="logNotes" name="logNotes" class="are arew" style="width:50%;" rows="6" cols="80"></textarea><span class="pos" id="warn" style="display:none">请限制在90个字符以内</span>
            	<input type="hidden" name="oldAuditStatus" id="oldAuditStatus" value="${offerGroup.auditStatus }" >
            </p>
            <p><label>&nbsp;</label><input id="activityID" type="button" class="hand btn83x23" value="<fmt:message key='tag.confirm'/>" /><input type="button" onclick="history.back();" class="hand btn83x23b" value='<fmt:message key="button.cancel"/>' />
...
<iframe src="${base}/consolelog/top10.form?entityId=${offerGroup.offerGroupId}&tableName=EB_OFFER_GROUP" width="100%" height="400" marginwidth="0" marginheight="0" frameBorder="no" framespacing="0" allowtransparency="true" scrolling="auto"></iframe>
