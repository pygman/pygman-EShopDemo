...
<script language="javascript" type="text/javascript">
	$(document).ready(function(){
		...
		$("input[id='addItemNoteConfirm']").click(function(){
	        ...
	        $.ajax({
	        	type:"post",
	         	url:"${base}/item/changeCardShowStatusAjax.form",
	         	data:(ajaxData),
    ...
    function publishContent(itemId){
        ...
        $.ajax({
        	type:"post",
         	url:"${staticPublishUrlPre}/static4ebiz/static/publishSingleContent.form",
         	data:(ajaxData),
         	success:function(responseText){
            	var result=eval("("+responseText+")");
             	if(result._status=="true"){
...
<div class="frameR"><div class="content">
    ...
    <input type="hidden" id="deleteAction" name="deleteAction" value="${base}/card/deleteCard.form"/>
    <input type="hidden" id="deleteCheckAction" name="deleteCheckAction" value="${base}/item/deleteCheck.form"/>

    <form id="form1" name="form1" action="${base}/card/listCard.form" method="post">
...
<!--             <input type="button" onclick="batchDown();" value="下架" class="hand btn60x20" /> -->
            ...
            <span class="r inb_a"><a href="${base}/item/addCatItemCard.form"  class="btn80x20">添加号卡</a></span>
        </div>
                    ...
                    <td>
                        <c:url value="" var="deleteItemById">
                         ...
                         <c:if test='${item.showStatus==0}'>
                          ...
                          <c:if test='${item.showStatus==1}'>
                         	<a href="${base}/card/editCard.form?&itemId=${item.itemId}" title="编辑基本信息">编辑</a>
                         </c:if>
