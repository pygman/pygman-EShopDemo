...
<div class="loc icon"><samp class="t12"></samp>当前位置：商品管理&nbsp;&raquo;&nbsp;<a href="<c:url value="/${path}/ecps/console/item/listEntity.form?showStatus=1"/>" title="商品录入/上下架">商品录入/上下架</a>&nbsp;&raquo;&nbsp;<span class="gray">查看商品</span>
    <a href="<c:url value="/${path}/ecps/console/item/listEntity.form?showStatus=1"/>" title="返回商品录入/上下架" class="inb btn120x20">返回商品录入/上下架</a>
</div>
<form action="${path}/item/updateItem.form" name="myForm" id="myForm" method="post">
...
<div id="tab_1" class="edit set">
    ...
    <p><label>状态：</label><input<c:if test="${ebItem.isNew==1}"> checked</c:if> id="featuredItem" name="featuredItem" type="checkbox" value="1" disabled="true"/>新品&nbsp;&nbsp;<input<c:if test="${ebItem.isGood==1}"> checked</c:if> id="featuredItem" name="featuredItem" type="checkbox" value="2" disabled="true"/>推荐&nbsp;&nbsp;<input<c:if test="${ebItem.isHot==1}"> checked</c:if> id="featuredItem" name="featuredItem" type="checkbox" value="3" disabled="true"/>特价
    </p>
