...
function checkCatNameAndMark(catId ,catName,catMark){
	...
	$.ajax({
		type:'post',
		url:'${path }/item/cat/checkCatName.form?catId='+catId+'&catName='+catName,
		dataType: 'json',
		async:false,
...
function checkCatMark(catId ,catMark){
	...
	$.ajax({
		type:'post',
		url:'${path }/item/cat/checkCatMark.form?catId='+catId+'&catMark='+catMark,
		dataType: 'json',
		async:false,
		complete:function(data) {
			if(data!=undefined && data.responseText !=undefined && data.responseText !=""){
...
<div class="frameR"><div class="content">
    ...
    <div class="loc icon"><samp class="t12"></samp><fmt:message key='menu.current.loc'/>：<fmt:message key='ItemMgmtMenu.title'/>&nbsp;&raquo;&nbsp;<a href="<c:url value="/${system}/item/cat/listCat.form?catType=1"/>" title="分类管理">分类管理</a>&nbsp;&raquo;&nbsp;<fmt:message key='menu.editCat'/>
    <a href="${backurl }" title="返回分类管理" class="inb btn80x20">返回分类管理</a>
    </div>
	<form id="form111" name="form111" action="${path }/item/cat/updateCat.form" method="post">
	    <div class="edit set">      
