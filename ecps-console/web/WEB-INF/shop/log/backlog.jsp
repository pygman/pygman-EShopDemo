...
<script type="text/javascript">
    ...
    function singleDel(logId, logTime){
        if(confirm("确定删除记录")){
        	...
        	$.ajax({
	    		...
	    		url:'${base}/log/login/getTime.form',
	    		...
	    		complete:function(data){
	    			...
	    			if(difflog>0) {
	    				$("#form1").attr("action","${base}/log/login/deleteBackLandLogById.form?id=" + logId);
	    	           	$("#form1").submit();
	    			} else {
	    				alert("不能删除3个月内的日志!");
	    			}
        ...
        if(confirm("确定删除这些记录")){
        	$.ajax({
	    		...
	    		complete:function(data){
		            ...
		            if(in3m&&out3m){
		        	$("#form1").attr("action","${base}/log/login/batchDelBackLandLogById.form");
		           	$("#form1").submit();	           	
    ...
    $(document).ready(function(){
    	...
    	$("#exportLoginLogID").click(function(){
    		if(checkDate()){
    			 $("#form1").attr("action","${base}/log/login/exportBackLandLog.form");
    		     $("#form1").submit();
    		     $("#form1").attr("action","${base}/log/login/listBackLog.form");
    		}
   		});
    });
</script>
...
<form id="form1" name="form1" action="${base}/log/login/listBackLog.form" method="post">
 	<div class="sch">
     	<p>
